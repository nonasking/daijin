...
# 사용법: bash flash-node.sh <dev1|dev2> <포트>
...
NODE="${1:?노드 이름 (dev1|dev2)}"; PORT="${2:?포트 (/dev/cu.usb...)}"
case "$NODE" in
...
arduino-cli compile --fqbn esp32:esp32:esp32s3 \
  --build-property "compiler.cpp.extra_flags=-DNODE_NAME=\"$NODE\" $FLAGS" 15-home-node || exit 1
